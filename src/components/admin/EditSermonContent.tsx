  ...
  useEffect(() => {
    ...
    const fetchSermonData = async () => {
      try {
        ...
        const data = await response.json();
        const sermon = data.data; // Extract sermon from the response
        
        // Convert Unix timestamp to date string for the input
        const dateObj = new Date(sermon.date * 1000); // Convert seconds to milliseconds
        const formattedDate = dateObj.toISOString().split('T')[0]; // Format as YYYY-MM-DD
        
        setFormData({
          ...sermon,
          date: sermon.date, // Keep the Unix timestamp for internal use
